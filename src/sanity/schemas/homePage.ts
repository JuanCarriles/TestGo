...
export default defineType({
  ...
  fields: [
    ...
    // ── ESPECIALIDADES ──
    defineField({
      name: 'especialidadesGrid',
      title: 'Encabezado de Especialidades',
      type: 'object',
      ...
      fields: [
        ...
        defineField({
          ...
          rows: 3,
          validation: (Rule) => Rule.required().max(300),
          initialValue: 'Contamos con un equipo multidisciplinario de excelencia para cuidar de tu salud y la de tu familia.',
        }),
      ],
    }),
      ...
      of: [
        {
          ...
          fields: [
            defineField({
              ...
              type: 'number',
              description: 'Número menor = aparece primero. Las obras se ordenan de menor a mayor.',
            ...
            defineField({
              ...
              type: 'image',
              options: { hotspot: true },
              description: 'Formato: PNG con fondo transparente o SVG (ideal). Tamaño recomendado: 300x80px (ratio horizontal ancho). El logo se muestra en un carrusel de 4 columnas, con un alto máximo de 40px. Evitar imágenes verticales o con fondo blanco.',
              fields: [
                defineField({
                  name: 'alt',
                  title: 'Texto alternativo',
    ...
    defineField({
      name: 'obrasSocialesBadge',
      title: 'Etiqueta superior',
      type: 'string',
      group: 'obrasSociales',
      description: 'Texto pequeño que aparece arriba del título. Ej: "Cobertura"',
      validation: (Rule) => Rule.required().max(50),
      initialValue: 'Cobertura',
    }),
    defineField({
      name: 'obrasSocialesTitulo',
      title: 'Título',
      type: 'string',
      group: 'obrasSociales',
      validation: (Rule) => Rule.required().max(80),
      initialValue: 'Obras Sociales Aceptadas',
    }),
    defineField({
      name: 'obrasSocialesTituloDestacado',
      title: 'Parte destacada del título',
      type: 'string',
      group: 'obrasSociales',
      validation: (Rule) => Rule.required(),
      initialValue: 'Aceptadas',
    }),
    defineField({
      name: 'obrasSocialesDescripcion',
      title: 'Descripción',
      type: 'text',
      group: 'obrasSociales',
      rows: 3,
      validation: (Rule) => Rule.required().max(300),
      initialValue: 'Trabajamos con las principales obras sociales y prepagas del país para tu tranquilidad.',
    }),
    defineField({
      name: 'obrasSocialesBotonTexto',
      title: 'Texto del botón',
      type: 'string',
      group: 'obrasSociales',
      description: 'Texto del botón que aparece debajo del carrusel de obras sociales.',
      validation: (Rule) => Rule.max(50),
      initialValue: 'Ver todas las obras sociales',
    }),
    defineField({
      name: 'obrasSocialesBotonEnlace',
      title: 'Enlace del botón',
      type: 'url',
      group: 'obrasSociales',
      description: 'URL externa a la que lleva el botón (ej: https://www.ejemplo.com/obras-sociales).',
      validation: (Rule) => Rule.required(),
    }),
    defineField({
      name: 'contacto',
